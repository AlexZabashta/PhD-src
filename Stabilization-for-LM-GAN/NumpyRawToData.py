...
def tour_cost(tours, cost_fun, data_in, distances):
    ...
    for i, city in enumerate(tours):
        if i == n - 1:
            ...
            if (cost_fun == "d"):
                #cost_i = distances[(str(data_in[tours[i]]), str(data_in[tours[0]]))]
                cost_i = distance(data_in[tours[i]], data_in[tours[0]])
            ...
            if (cost_fun == "d"):
                #cost_i = distances[(str(data_in[tours[i]]), str(data_in[tours[i + 1]]))]
                cost_i = distance(data_in[tours[i]], data_in[tours[i + 1]])
                #print(cost_i, cost_i1)
...
def TSP(data_in, method_type):
    ...
    # print(data_in.shape)
    # out_zeros = data_in[np.where(data_in[:, 16] == 0.0)][:-1]
    # out_ones = data_in[np.where(data_in[:, 16] == 1.0)][:-1]
...
def grid_salesman_search(data_in: np.ndarray):
    min_cost = 1000000001
    n = len(data_in.T) - 1
    qwerty = data_in[:, :-1].T
...
def correlation_method(data: np.ndarray):
    ...
    print("Correlation method: " + str(sorted_indexes))
...
def spearman_cor(data: np.ndarray):
    ...
    for i in range(0, (num_features)):
        ...
        cur_spearman = spearmanr(st1, st2)[0]
    ...
    for i, _ in pairs:
        sorted_indexes.append(i)
    print("Spearman correlation method: " + str(sorted_indexes))
    sorted_indexes.append((num_features))
    # print(sorted_data.shape)
    # print(len(sorted_indexes))
...
def writeData(name_in: str, zero_num: int, one_num: int, zero_in: np.ndarray, one_in: np.ndarray) -> bool:
    ...
    # print("Hill climbing method: " + str(hill_climbing(data_in)))
    # print("Grid Salesman Search: ")
    zero_grid, one_grid = grid_salesman_search(data_in)
    zero_tsp, ones_tsp = TSP(data_in.T, 1)
    #TSP(data_in.T, 2)
